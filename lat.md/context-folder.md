...
The gateway validates that `output_dir` is an existing absolute directory and adds its policy only to model input, never to displayed or persisted user text. The compatibility API path uses an equivalent system message. [[tests/session-output.test.ts]] verifies destination normalization, while [[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.test.tsx]] verifies recovery preserves the turn-scoped directory.
