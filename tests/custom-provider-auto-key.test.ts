...
describe("setModelConfig — known-host custom provider auto-api-key (issue #260)", () => {
  ...
  it("writes a named custom-provider key for a private gateway model", async () => {
    writeBaseFiles(
      "CUSTOM_PROVIDER_INTERNAL_GATEWAY_KEY=sk-private-gateway-key\n",
      `${SEED_YAML}providers:
  stale-gateway:
    name: "Stale Gateway"
...
`,
    ...
    const out = readFileSync(join(testHome, "config.yaml"), "utf-8");
    expect(out).toContain('api_key: "sk-private-gateway-key"');
  });
