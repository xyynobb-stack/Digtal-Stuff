...
[[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts#ensureDashboardRuntimeSession]] resolves a generic custom selection through the local-only `model.resolve` RPC before `session.create`, then sends `model` and `provider` as the gateway's per-session build override. `session.create` and `session.resume` return the effective identity in `info.model/provider`; chat consumes that identity directly and uses the local-only `model.identity` RPC after an actual switch. The message path never calls `model.options`, so provider discovery, pricing, metadata, or an unavailable network cannot delay the first turn.
