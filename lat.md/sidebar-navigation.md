...
The Simplified Chinese locale supplies the complete menu and confirmation vocabulary, including 置顶、取消置顶、重命名、移动到项目、删除 and project-picker labels, so i18n does not fall back to English on an otherwise Chinese desktop.
...
Employee phone provisioning has stricter semantics than an administrative Profile switch. Once a newly configured employee becomes active, [[src/renderer/src/screens/Layout/chatRuns.ts#provisionEmployeeRunTransition]] retires every open run, including background work, and creates one blank run for the employee Profile. The shell aborts those retired run ids before removing their tabs, so an old employee's transcript cannot remain visible or switch the window back to that identity. Persisted history remains in its Profile-scoped database and is not deleted.
