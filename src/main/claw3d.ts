...
export function startAdapter(): boolean {
  ...
  proc.on("close", (code) => {
    if (code && code !== 0 && !adapterError) {
      adapterError = `JingYuAI adapter exited with code ${code}`;
    }
    adapterProcess = null;
    cleanupPid(ADAPTER_PID_FILE);
  });
...
export function startAll(profile?: string): {
  ...
  if (!adapterOk) {
    return { success: false, error: "Failed to start JingYuAI adapter" };
  }
