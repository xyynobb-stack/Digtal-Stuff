...
interface UseDashboardChatTransportArgs {
  ...
  /** Receives the authoritative title generated by the Gateway after the
   *  first exchange. The Gateway persists it before emitting the event; this
   *  callback updates only the live Renderer state. */
    ...
    (event: DashboardStreamEvent): void => {
      ...
      if (event.type === "session.title") {
        ...
        const targetSessionId = String(
          payload.session_id ?? storedSessionIdRef.current ?? "",
        ).trim();
        const currentStoredSessionId = storedSessionIdRef.current;
        // Auto-title runs in a background thread. A result from a retired
        // session can therefore arrive after this Chat instance has resumed a
        // different stored session, especially across reconnects. Never apply
        // that late title to the new tab or sidebar row.
        if (
          !title ||
          !targetSessionId ||
          (currentStoredSessionId && currentStoredSessionId !== targetSessionId)
        ) {
          logDashboardEvent(event, "dropped", runtimeSessionId);
          return;
        }
        onSessionTitleRef.current?.(targetSessionId, title);
