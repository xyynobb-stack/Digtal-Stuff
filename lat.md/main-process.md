...
The desktop gives every Agent a fixed 旌渝 company identity in model-visible prompts without renaming Hermes-compatible runtime identifiers.

`patchJingYuAgentIdentitySource` in `scripts/apply-offline-runtime-overlays.mjs` updates the fallback identity, help guidance, platform hints, profile context, default and Docker SOUL templates, doctor-created SOUL, MoA aggregation, profile-description prompts, and model-visible desktop tool descriptions. The managed prompt identifies the assistant as a digital employee provided by 旌渝 and forbids translating, transliterating, or rewriting that brand. `scripts/prepare-dev-agent.mjs` applies the same idempotent overlay to development startup and every existing Profile SOUL, while `scripts/prepare-offline-runtime.mjs` applies it before the packaged Runtime archive is created.

Packaged startup migrates only exact recognized Hermes, JingYu Agent, and JingYuAI identity sentences in every existing SOUL file, appends the managed identity when absent, and preserves every other user-authored instruction. Re-running the migration makes no further change. A separately marked SOUL rule makes progress updates, tool-call explanations, errors, and final answers use Simplified Chinese when the latest user message is Chinese; provider-native reasoning, code, commands, identifiers, and tool parameters remain untouched. The actual request body, model route, request headers, tool arguments, CLI names, environment variables, and internal `hermes-agent` Skill name are unchanged.
...
Employee provisioning assigns one trace id to gateway stop, spawn, process, and health-check events. During that bounded attempt, Python import timings follow a matching marker in the Profile's `gateway-stderr.log`, exposing delays before runtime logging and PID publication without adding overhead to ordinary gateway starts.
...
`scripts/prepare-offline-runtime.mjs` reads Skills only from `resources/starter-skills` and writing templates only from `resources/starter-writing-templates`, then stages them under `build/offline-runtime/preset-content`. Release jobs call `syncRepositoryPresetSkills` and `syncRepositoryPresetWritingTemplates` from `scripts/apply-offline-runtime-overlays.mjs` to refresh both inventories before packaging; template synchronization replaces the generated template staging directory so deleted presets cannot linger. On Windows it copies directories and files explicitly instead of using Node 22 `fs.cpSync`, which can terminate without an exception while recursively copying Chinese template directory names. macOS preparation reads the same repository inventories directly. Builder-profile Skills and templates are never packaged.
...
The schedule creation form requires one saved model and passes its model/provider pair through preload and IPC to [[src/main/cronjobs.ts#createCronJob]], which stores the pair with the job through Hermes CLI `--model` and `--provider` flags. Chat model changes therefore do not leave scheduled execution dependent on an empty global default. An ordinary local task may also select one profile-owned writing template; the form preserves “no template” as its default, embeds the selected template's stable library directory and current absolute path in the prompt, protects the source from overwrite, and activates `xlsx` for Excel templates. The same inline import action used by Discover can add and immediately select a template without closing the task form. Remote tasks disable this local-path feature. For legacy local or named-profile SSH jobs whose stored model is empty, [[src/main/cronjobs.ts#triggerCronJob]] first applies the currently selected model with `cron edit`, stops if that edit fails, and only then executes `cron run`.

The custom-frequency form never exposes cron syntax. Users enter a month, calendar day, hour, and minute; the Renderer validates numeric ranges and month-specific day limits, then converts the values to the five-field annual cron expression required by the existing CLI. February 29 remains valid for leap-year runs, while impossible dates and times keep creation disabled.
...
Local-delivery jobs store either the absolute output root selected in the schedule form or the displayed profile default `<HERMES_HOME>/cron/output`. The Agent writes each audit run below `<stored root>/<job id>/`. Persisting the displayed default as well as an explicit selection gives both paths identical deliverable behavior. Because the path belongs to the persisted cron job, scheduled sessions never inherit a normal chat's context folder. `scripts/patch-cron-output-directories.mjs` reapplies this Agent-core extension whenever Windows or macOS offline runtimes are prepared, so packaged builds retain the behavior after their source runtime is refreshed.

### Generated deliverable destination

The selected cron output root is both the audit-record root and the destination for generated user-facing files, so a successful scheduled report does not require a manual chat download.

At execution time [[build/offline-runtime/hermes-agent/cron/scheduler.py#_resolve_job_deliverable_output_dir]] revalidates the persisted absolute directory and injects its resolved path into the model-only task prompt. When the job has no explicit `workdir`, the same directory becomes its session and terminal working directory so skills that emit relative filenames also land there; an explicit `workdir` remains authoritative for source-oriented jobs, while the injected absolute destination still directs their deliverables. Missing or inaccessible output directories fail visibly instead of silently falling back to a temporary attachment location. Audit Markdown remains isolated at `<selected root>/<job id>/<timestamp>.md`. `scripts/patch-cron-output-directories.mjs` anchors the Agent branch by its unique surrounding comment instead of replacing the first `_job_workdir` occurrence, because older schedulers also contain an indented no-agent branch; it also repairs the malformed intermediate form produced by the earlier patch. [[tests/cron-output-directories-overlay.test.ts]] verifies exact-path injection, branch-safe working-directory fallback, malformed-runtime recovery, and patch idempotence.
