...
function transformFile(agentRoot, relativePath, transform) {
  const filePath = path.join(agentRoot, ...relativePath.split("/"));
...
export function patchCronDeliverableOutputSource(
  source,
  filePath = "cron/scheduler.py",
) {
  let patched = source;
  const malformedNoAgentWorkdir = `        _configured_workdir = (job.get("workdir") or "").strip() or None
    _job_workdir = _configured_workdir or _resolve_job_deliverable_output_dir(job)`;
  if (patched.includes(malformedNoAgentWorkdir)) {
    patched = patched.replace(
      malformedNoAgentWorkdir,
      `        _job_workdir = (job.get("workdir") or "").strip() or None`,
    );
  }

  if (!patched.includes("def _resolve_job_deliverable_output_dir")) {
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:`,
  ...
  const resolvedAgentWorkdir =
    `    _configured_workdir = (job.get("workdir") or "").strip() or None\n` +
    `    _job_workdir = _configured_workdir or _resolve_job_deliverable_output_dir(job)`;
  if (!patched.includes(resolvedAgentWorkdir)) {
    const agentWorkdirAnchor =
...
export function patchCronOutputDirectories(agentRoot) {
  ...
  transformFile(
    agentRoot,
    "cron/scheduler.py",
    patchCronDeliverableOutputSource,
  );
