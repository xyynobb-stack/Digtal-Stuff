...
[[src/main/installer.ts#initializeBundledRuntime]] starts the first-launch copy only after Electron has created the application window. The copy uses asynchronous filesystem operations and is shared by all callers; local installation checks await it, preventing both main-process startup crashes and access to a partially copied Agent runtime.

`scripts/prepare-offline-runtime.mjs` copies the tested Python installation and Hermes Agent (including its virtual environment) into `build/offline-runtime`; `electron-builder.yml` places that directory under `resources/hermes-runtime`. The staging script writes `desktop-runtime-build.json` with a unique build ID, and the Windows release workflow stamps it with the release commit. In packaged mode, [[src/main/installer.ts#initializeBundledRuntime]] combines that marker with the Electron app version before comparing it with the writable copy under `userData`; every application upgrade therefore refreshes the complete managed Agent and Python trees even if a staging marker was accidentally reused. Treating all Python modules as one compatibility unit prevents a new `run_agent.py` from importing symbols from an older tool module. Same-build launches still repair a missing `venv`, runtime, or launcher, repair `pyvenv.cfg` for the relocated Python path, and use the bundled repository as the active runtime. Development installs retain the existing `%LOCALAPPDATA%\\hermes` discovery behavior.
