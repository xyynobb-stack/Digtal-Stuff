export default {
  title: "אופיס",
  subtitle: "הסוכנים שלכם בעבודה, בשידור חי בתלת-ממד",
  loadingAgents: "טוען סוכנים...",
  noAgents: "לא נמצאו סוכנים.",
  refresh: "רענון",
  agentCount_one: "{{count}} סוכן",
  agentCount_other: "{{count}} סוכנים",
  selectAgentHint: "לחצו על סוכן כדי להתמקד בו",
  checkingStatus: "בודק את מצב Claw3D...",
  setupTitle: "הגדרת Claw3D",
  installTitle: "מגדיר את Claw3D",
  processLogs: "יומני תהליך",
  noLogs: "אין יומנים עדיין. הפעילו את השירותים כדי לראות פלט.",
  loadingClaw3d: "טוען את Claw3D...",
  installClaw3d: "התקנת Claw3D",
  setupFailed: "ההגדרה נכשלה",
  startFailed: "הפעלת Claw3D נכשלה",
  portInUse: "פורט {{port}} בשימוש. שנו אותו בהגדרות כדי להפעיל.",
  websocketUrl: "כתובת WebSocket",
  viewOnGithub: "צפייה ב-GitHub",
  waitingToStart: "ממתין להפעלה...",
  starting: "מפעיל...",
  openInBrowser: "פתיחה בדפדפן",
  viewLogs: "צפייה ביומנים",
  portInUseWarning:
    "פורט {{port}} בשימוש. אנא שנו את הפורט בהגדרות או עצרו תהליכים אחרים.",
  close: "סגירה",
  cannotLoadClaw3d: "לא ניתן לטעון את Claw3D",
  startingClaw3dService: "מפעיל את שירות Claw3D...",
  clickToStart: 'לחצו על "הפעלה" כדי להריץ את Claw3D',
  setupDesc1:
    "‏Claw3D היא סביבת ויזואליזציה תלת-ממדית לסוכני JingYuAI שלכם. היא מאפשרת לכם לראות את הסוכנים שלכם עובדים במרחב משרדי אינטראקטיבי.",
  setupDesc2:
    "לחצו למטה כדי להוריד ולהגדיר את Claw3D אוטומטית. פעולה זו תשכפל את המאגר ותתקין את כל התלויות.",
  // Agent details sidebar
  statusLabel: "סטטוס",
  modelLabel: "מודל",
  providerLabel: "ספק",
  gatewayLabel: "שער",
  gatewayRunning: "פעיל",
  gatewayStopped: "מופסק",
  status_working: "עובד",
  status_idle: "בהמתנה",
  status_error: "שגיאה",
  employee: "עובד",
  ceo: 'מנכ"ל',
  makeCeo: 'מינוי למנכ"ל',
  removeCeo: 'הסרה מתפקיד מנכ"ל',
  // Enterable building interiors
  enter_office: "כניסה למשרד",
  enter_bank: "כניסה לבנק",
  enter_showroom: "כניסה לאולם התצוגה",
  exitToCity: "יציאה לעיר",
  showroomCardColor: "צבע הרכב",
  showroomCardHint:
    "הסתובבו באולם התצוגה — רכישת מכוניות לסוכנים שלכם תגיע בקרוב.",
  // Space representatives (bank teller interaction menu)
  walkMode: "הסתובבות",
  walkModeExit: "יציאה מהליכה (Esc)",
  walkHint: "WASD / חצים לתנועה · Shift לריצה · Esc ליציאה",
  you: "אתה",
  repBankTeller: "פקיד בנק",
  spaceBank: "בנק",
  repPanelAgentLabel: "סוכן",
  repPanelPickAgent: "בחרו סוכן…",
  repActionCheckBalance: "בדיקת יתרה",
  repActionAccountStatus: "בדיקת מצב חשבון",
  repActionCreateAccount: "פתיחת חשבון",
  repActionSendMoney: "שליחת כסף לסוכן",
  repComingSoon: "בקרוב",
  repLoading: "עובד…",
  repErrorGeneric: "משהו השתבש. נסו שוב.",
  repStatusSignedOut: "התחברו לחשבון JingYuAI כדי להשתמש בבנק.",
  repStatusUnlinked: "סוכן זה עדיין לא מקושר לסוכן ענן. התחברו ונסו שוב.",
  repWalletsNone: "אין עדיין חשבונות — פתחו אחד כדי להתחיל.",
  repBadgeTransactable: "ניתן לביצוע עסקאות",
  repBadgeReceiveOnly: "קבלה בלבד",
  repBalanceNoTransactable: "לסוכן זה אין עדיין חשבון עסקאות — פתחו אחד תחילה.",
  repBalanceEmpty: "אין עדיין יתרות אסימונים.",
  repBalanceTotal: 'סה"כ',
  repCreateSuccess: "החשבון נפתח",
  repCreateExists: "לסוכן זה כבר יש חשבון בנק.",
  repStatusForeign: "סוכן זה מקושר לחשבון JingYuAI אחר.",
} as const;
