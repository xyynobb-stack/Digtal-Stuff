export default {
  ...
  analytics: {
    label: "发送匿名使用情况分析",
    hint: "通过向项目的分析服务发送匿名、聚合的使用数据来帮助改进 JingYuAI。您可以随时关闭。",
    disclosure: {
      ...
      endpoint:
        "数据将发送至 JingYuAI 分析服务（analytics.hermesone.org）。已禁用会话录制和页面浏览自动捕获。",
      notCollected:
  ...
  hardwareAcceleration: {
    ...
    on: "始终开启",
    off: "始终关闭",
    hint: "在可用时使用 GPU 进行界面渲染。",
    envOverride:
      "当前设置被 HERMES_DISABLE_GPU 环境变量覆盖；移除该变量后此设置才会生效。",
    restartToApply: "重启软件后生效。",
    restartNow: "立即重启",
    saveFailed: "无法保存该设置。",
  },
  theme: {
  ...
  poolHint:
    "为同一提供商添加多个 API Key,以便自动轮换和负载均衡。JingYuAI 会在它们之间轮流使用。",
  add: "添加",
  remove: "移除",
  keyLabel: "密钥",
  empty: "(空)",
  dataSection: "数据",
  dataHint: "导出或导入你的 JingYuAI 配置、会话、技能和记忆。",
  backingUp: "正在备份...",
  exportBackup: "导出备份",
  importing: "正在导入...",
  importBackup: "导入备份",
  ...
  migrationDesc:
    "在 <code>{{path}}</code> 发现 OpenClaw。你可以将配置、API Key、会话和技能迁移到 JingYuAI。",
  migrationDismiss: "不再显示",
  migrating: "迁移中...",
  migrateToHermes: "迁移到 JingYuAI",
  skip: "跳过",
  ...
  modeLocal: "本地",
  modeRemote: "远程",
  modeLocalHint: "使用本机安装的 JingYuAI",
  modeRemoteHint: "连接到网络或云服务器上的 JingYuAI API",
  remoteUrl: "远程服务器地址",
  remoteUrlHint:
    "JingYuAI API 服务器地址（需开放 /health 和 /v1/chat/completions）",
  remoteApiKey: "API 密钥",
  remoteApiKeyHint:
  ...
  serverConfigHint:
    "你已连接到远程 JingYuAI 服务器。模型选择、提供商 API Key 和凭据均在服务器的 <code>~/.hermes/.env</code> 和 <code>config.yaml</code> 中管理。请在主机上编辑（例如 <code>docker exec -it hermes vi /opt/data/.env</code>）然后重启容器。",
  connectionMode: "模式",
  switchedToLocal: "已切换到本地模式",
  communityLinksHint: "了解旌渝社群或寻求帮助",
} as const;
